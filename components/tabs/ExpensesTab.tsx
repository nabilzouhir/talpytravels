...
export default function ExpensesTab({ activities }: Props) {
  const expenses = activities
    .filter((a) => a.price != null && a.price > 0 && a.paid_by != null)
    .sort((a, b) => {
      if (a.day_number != null && b.day_number != null && a.day_number !== b.day_number) {
  ...
  const totalPesciolino = expenses
    .filter((a) => a.paid_by === "pesciolino")
    .reduce((sum, a) => sum + (a.price ?? 0), 0);
  const totalTalpina = expenses
    .filter((a) => a.paid_by === "talpina")
    .reduce((sum, a) => sum + (a.price ?? 0), 0);
